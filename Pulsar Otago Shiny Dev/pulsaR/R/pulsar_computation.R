...
fsm_split_peaks <- function(peak_points, peak_split_depth, rescaled_conc)
{
  ...
  while(main_walker <= n_observations)
  {
    ...
    #############################################################
    #Debug
    # debug_list = list(main_walker = main_walker,
    #                   next_loc = next_loc,
    #                   current_state = current_state,
    #                   current_conc = current_conc,
    #                   current_peak_loc = current_peak_loc,
    #                   current_peak_conc = current_peak_conc,
    #                   current_dip_loc = current_dip_loc,
    #                   current_dip_conc = current_dip_conc,
    #                   delta_conc = delta_conc)
    # 
    # print(paste(debug_list['main_walker'], debug_list['delta_conc'], debug_list['current_state'], sep = ", "))
    # ##############################################################
    # 
    

    if (current_state == st_seeking) {
    # The next point is marked as in a pulse
    # Change state, and set current max concentration to the value
    # of that next point.
      ...
      # state is climbing, meaning we are in a pulse
      # if delta_conc is positive it means that the next point is higher than this one,
      # we are moving upward within the pulse.         
      # If the next concentration is larger than current max, it becomes current max
      if (delta_conc > 0) {
        if (next_conc > current_peak_conc){
          current_peak_loc <- next_loc
          current_peak_conc <- rescaled_conc[current_peak_loc]
        }
      #  Otherwise, the next concentration in the pulse is lower than this one,
      #  we are moving downward within the pulse. We begin to check whether we have
      # fallen peak_split_depth units while still in the pulse. If this happens
      # we break this pulse up into two peaks. We begin looking for the lowest
      # point in the sequence. That will become the break point.
